...
`aistar` is itself a coding agent, and its own development is conducted with heavy use of LLM coding agents (Aider, Claude, Gemini, etc.). This creates a unique meta-problem:
...
Popularised by llama.cpp and other AI-native projects. Works well for stable conventions but degrades for per-task context because the file grows unbounded. `aistar` uses `CONTRIBUTING.md` for stable workflow conventions and `TASKS/` for per-task ephemeral context — separating the two concerns.
